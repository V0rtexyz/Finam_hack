...
_LABEL_SYSTEM_PROMPT = (
    "Ты финансовый аналитик и придумываешь короткие названия для кластеров финансовых новостей. "
    "Каждое название помогает инвестору мгновенно понять тему и драйвер движения."
)

_LABEL_GENERIC_SET = {
    "нет релевантного тренда",
    "нет релевантного тренда.",
    "итоги дня",
    "итоги дня на рынке",
    "обзор рынка",
...
def _ensure_unique_label(label: str, terms: list[str], cid: int, used: set[str]) -> str:
    ...
    if slug not in used:
        used.add(slug)
        return candidate
    anchor = _pick_anchor_term(terms) or f"кластер {cid}"
    base = candidate
...
def label_clusters_with_llm(cluster_terms: dict[int, list[str]]) -> dict[int, str]:
    ...
    user_message = (
        "Ниже перечислены кластеры новостей с ключевыми словами. "
        "Для каждого id придумай уникальное русскоязычное название из 3-6 слов, "
        "которое отражает драйвер или тему для инвестора. Используй тикеры или сектора, когда это уместно. "
        "Не используй расплывчатые фразы вроде 'нет релевантного тренда' или 'итоги дня'. "
        "Ответ верни строго в JSON: {\"labels\": [{\"id\": int, \"title\": \"...\"}, ...]} без дополнительных комментариев."
        f"Кластеры:{user_payload}"
    )
    try:
        ...
        print(f"Ошибка LLM для названий кластеров: {exc}")
        llm_labels = {}
    labels: dict[int, str] = {}
    used: set[str] = set()
    for cid in cluster_ids:
